...
public:
	...
	Cache() {
		...
		los = false;
		food = true;
		cache_t.loadFromFile("images/entities/interactable/food.png");
		...
		if (target != NULL) {
			...
			if (target_y > 0) {
				...
				if ((_grid[(int(x)/16)][int(y-1)/16] == 0) && (_grid[(int(x+15)/16)][int(y-1)/16] == 0)) {
					if (!(x_stuck)) {
						target_y -= float(0.5 * cos(direction));
					}
					else {
						target_y -= 0.5;
					}
	...
	public:
		...
		void get_action(controller& _controller) {
			...
			if (sf::Keyboard::isKeyPressed(sf::Keyboard::Space))
			{
				...
				for (int i = 0; i < _controller.cachelist.get_size(); i++) {
					if ((abs(_controller.cachelist.get_cache(i)->x - (x+7)) < 23) && (abs(_controller.cachelist.get_cache(i)->y - (y+7)) < 23) && (_controller.cachelist.get_cache(i)->food)) {
						hunger += 25;
						_controller.cachelist.get_cache(i)->food = false;
					}
				}
			}
...
int main()
{
    ...
    while (window.isOpen())
    {
		...
		player_health.setPosition(the_sam.x-320, the_sam.y-240);
